...
show "Updating system and installing required packages..." "progress"
apt update && apt install -y sudo
sudo apt update && sudo apt install -y python3 python3-venv python3-pip curl wget screen git lsof nano unzip
if [ $? -ne 0 ]; then
    show "Failed to update system or install required packages." "error"
    exit 1
fi
show "System updated and required packages installed successfully."
